...
**Changes:**
- `code/exp0_inn_train.py`: Imported `ImageSecretLoss` from `losses`, instantiated it after `INNCodec` construction with default weights (`yuv_w=1.5, lpips_w=1.0, ffl_w=1.5, secret_weight=args.bit_weight`), replaced the inline `F.l1_loss` + `F.binary_cross_entropy_with_logits` block with a single `img_loss_mod(...)` call wrapped in `autocast(enabled=False)` for fp32 stability of LPIPS/YUV/FFL, and extended the `metrics.jsonl` per-step record with `l_yuv`, `l_lpips`, `l_ffl`, `l_quality`, and `quality_alpha`.
...
```python
    # Perceptual host loss: YUV + LPIPS + FFL, with BCE bit pressure baked in.
